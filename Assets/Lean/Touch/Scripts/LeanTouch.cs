...
namespace Lean.Touch
{
	...
	public partial class LeanTouch : MonoBehaviour
	{
		public float Force;
		public bool isSwipe;
		[SerializeField] public GameObject[] NextStone;
				...
				if (finger.Up == true)
				{
					...
					if (finger.Age <= TapThreshold)
					{
						if (finger.SwipeScreenDelta.magnitude * ScalingFactor < SwipeThreshold)
						{
							finger.Tap       = true;
							finger.TapCount += 1;
							Debug.Log("Bu bir tap:" + finger.Age);
						}
						else
						{
							finger.TapCount = 0;
							finger.Swipe    = true;
							Debug.Log("Bu bir swipe:" + finger.Age);
							isSwipe = true;
							Force = finger.Age;
		...
		public void ThrowTheStone()
		{
			...
			StoneRB.AddForce(transform.forward * Force * 400);
			StoneRB.AddForce(transform.up * Force * 1000);
